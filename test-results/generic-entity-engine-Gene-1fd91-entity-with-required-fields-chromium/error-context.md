...
- Location: tests/e2e/generic-entity-engine.spec.js:76:3
...
```ts
  ...
  5   |   let csrfToken = '';
  6   | 
  7   |   // Helper to create a unique ID for test isolation
  8   |   function uniqueId(prefix) {
  9   |     return `${prefix}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  10  |   }
  11  | 
  12  |   // Fetch CSRF token from home page before running tests
