...
export async function createMysqlSchema(connection) {
  ...
  await connection.query(`
    CREATE TABLE IF NOT EXISTS entity_type_fields (
      ...
      label VARCHAR(255) NOT NULL,
      field_type ENUM('text','textarea','number','date','url','links','select','radio','status','priority','checkbox','recurrence','emoji','emojis','duration','timebox') NOT NULL,
      field_options JSON,
  ...
  await connection.query(`
    ...
    MODIFY COLUMN field_type ENUM('text','textarea','number','date','url','links','select','radio','status','priority','checkbox','recurrence','emoji','emojis','duration','timebox') NOT NULL
  `);
