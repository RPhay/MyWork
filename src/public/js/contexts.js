...
function initSubTabs() {
  ...
  nav.addEventListener("click", (e) => {
    ...
    if (target === "system-database") {
      loadSystemDatabaseSubpanel();
    } else if (target === "database" && selectedContextId) {
...
async function removeSystemDbConfig() {
  ...
  try {
    // For system database, we don't have a DELETE endpoint. We'll save empty config or reset to env config.
    // For now, just show a message that this would require server-side handling.
