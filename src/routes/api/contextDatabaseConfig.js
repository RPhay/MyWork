...
router.put('/:contextId', async (req, res) => {
  try {
    ...
    logger.error('Error saving context database config:', errorDetails);

    // Return detailed error to browser so user can see what failed
    const responseMessage = error.message || 'Failed to save database config';
...
// Remove a context's database connection
router.delete('/:contextId', async (req, res) => {
  try {
    await contextDatabaseConfigService.removeDbConfig(req.params.contextId);
    res.json({ success: true, message: 'Database connection removed' });
  } catch (error) {
