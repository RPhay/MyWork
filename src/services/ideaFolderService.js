...
export async function getAllFolders(contextId) {
  return await db.query('SELECT * FROM idea_folders WHERE context_id = ? ORDER BY name ASC', [contextId]);
}

export async function getFolderById(id) {
  const folder = await db.queryOne('SELECT * FROM idea_folders WHERE id = ?', [id]);
  if (!folder) {
...
async function getDescendantIds(id) {
  const all = await db.query('SELECT id, parent_id FROM idea_folders');
...
export async function createFolder(data, contextId) {
  ...
  const folderId = await db.insert(
    'INSERT INTO idea_folders (name, parent_id, context_id) VALUES (?, ?, ?)',
...
export async function updateFolder(id, data) {
  ...
  await db.update(`UPDATE idea_folders SET ${setClauses.join(', ')} WHERE id = ?`, values);

  return getFolderById(id);
}

export async function deleteFolder(id) {
  const affectedRows = await db.deleteRecord('DELETE FROM idea_folders WHERE id = ?', [id]);
