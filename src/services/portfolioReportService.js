...
/**
 * Everything placed on the priorities board, whatever its type.
 *
 * Membership is a field on the record (`on_board`), so what the board shows is
 * the record ITSELF, not a copy of it - dropping something here is always a
 * reference, and its title and fields stay owned by its own page. The bay a row
 * sits in is its own status, so moving it between bays is a real change to the
 * record, which is the one edit the board is meant to allow.
 */
export async function getBoardItems(contextId = null) {
  ...
  for (const type of types) {
    ...
    for (const entity of await entityService.getAllEntities(type.slug, contextId)) {
      if (!entity.fields?.on_board) continue;
      items.push({
        id: entity.id,
        title: entity.title,
        typeSlug: type.slug,
        ...
        statusFieldKey: statusField?.field_key || null,
