...
- **Row editors (Categories, Goals, Todos, Tasks, Tickets, Ideas, Projects, Templates) autosave.** There is no Save button. `genericEntity.js`'s `markChanged()` debounces a save `AUTO_SAVE_DELAY_MS` (800ms) after the last change and fires it via the `entity-autosave-due` event, which `generic-entity-init.js` listens for per type slug and turns into the same create-or-update call the old Save button made. **Revert is the only manual control left** - it discards whatever hasn't saved yet (`GenericEntity.discardChanges()`) and reloads the stored record; it stays disabled until something changes, same as before. Switching to a different record, or closing the editor, flushes a pending autosave immediately rather than losing it (`populate()` / `close()` in `genericEntity.js`) - Revert is exempt because it calls `discardChanges()` first, which empties `hasChanges` before either of those ever sees it.
- **Every other editor** (Settings > Entity Types, Dailies, Contexts, Priority Board, Theme, Data Sources) still uses the explicit **Save / Revert** pair. **Save starts disabled** and enables on the first change. `populate()`-equivalents must reset `disabled = true`; resetting only the dirty flag left the button enabled for every item opened afterwards. **Saving never closes an editor**, and neither does **Revert** - which reloads the stored record rather than discarding the editor. Both buttons are disabled until something changes. An editor is closed by clicking its row again.
